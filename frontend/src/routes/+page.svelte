...
<div class="flex h-full flex-row gap-5 p-5">
	<div class="md:w-5/7 mx-auto flex h-full w-full flex-col gap-5 pb-10 pl-5 lg:w-3/5 xl:w-2/3">
		...
		<div class="flex flex-col gap-2">
			<div class="flex flex-row gap-2 flex-wrap">
				{#each attached_files as url}
					<button on:click={() => remove_attached_file(url)} class="duration-400 pb-2 text-nowrap text-gray-400 transition hover:scale-90">[{file_to_url[url]}]</button>
				{/each}
			</div>
			<div class="flex flex-row gap-2">
				<p class="text-3xl">></p>
				<input bind:this={input_element} bind:value={input} on:keydown={handle_input_keydown} disabled={loading} class="flex-grow border-b bg-black px-3 pb-2 {loading ? '' : ''}  duration-400 text-white transition focus:outline-none" placeholder="" />
				<button on:click={submit_and_query} class="duration-400 pb-2 transition {loading || !input ? 'text-gray-400' : 'hover:scale-110'} text-3xl" disabled={loading || !input}><pre>[↵]</pre></button>
				<input type="file" disabled={loading} id="fileInputElement" on:change={handle_file_input} class="hidden min-w-10" />
				<label for="fileInputElement" class="{loading ? 'text-gray-400' : ' hover:scale-110'} mine-w-10 duration-400 cursor-pointer text-nowrap pb-2 transition text-3xl"><pre>[+]</pre></label>
			</div>
		</div>
	</div>
